  ...
  for (let window of windows) {
    for (let tab of window.tabs) {
      for (let subtab of tab.window.tabs) {
        ...
        highlightBrowser(viewFor(subtab.window));
      }
    }
  }
}
...
tabs.on("activate", function(aTab) {
  highlightBrowser(viewFor(aTab.window));
});

refreshWindows();
Services.obs.addObserver(refreshWindows, "priv8-refresh-needed", false);

function highlightBrowser(aWindow) {
  let win = viewFor(aWindow);
  if (!isBrowser(win)) {
    return;
  }

  let browser = win.gBrowser.getBrowserForTab(win.gBrowser.selectedTab);
  priv8.highlightBrowser(browser);
}
...
function createMenuItem(aWindow, aPopup, aSandbox) {
  ...
  item.addEventListener('command', function() {
    ...
    if (priv8.configureWindowByName(tab, browser.contentWindow, aSandbox)) {
      ...
      priv8.highlightBrowser(aWindow.gBrowser.getBrowserForTab(tab));
    }
  });
}
