...
function refreshButtonIcons(aWindow) {
  if (button) {
    let docShell = aWindow.gBrowser.contentWindow.QueryInterface(Ci.nsIInterfaceRequestor)
                                                 .getInterface(Ci.nsIDocShell);
    let attr = docShell.getOriginAttributes();
    if (priv8.getSandboxName(priv8.getSandboxFromOriginAttributes(attr)) == null) {
      button.icon = {
...
function configureWindow(aWindow) {
  ...
  popup.addEventListener('popupshown', function(e) {
    ...
    let item;
    let id = priv8.getSandboxFromOriginAttributes(docShell.getOriginAttributes());
    if (id) {
...
function createMenuItem(aWindow, aPopup, aSandbox) {
  ...
  item.addEventListener('command', function() {
    ...
    if (priv8.getSandboxFromWindow(browser.contentWindow) != aSandbox) {
