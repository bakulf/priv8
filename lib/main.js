...
require("sdk/tabs").on("open", function(aTab) {
...
function createMenuItem(aWindow, aPopup, aSandbox) {
  ...
  item.setAttribute("label", aSandbox ? aSandbox : "Default");
  ...
  item.addEventListener('command', function() {
    ...
    let appId = priv8.appIdForSandbox(aSandbox);
    if (docShell.appId == appId) {
      return;
    }

    docShell.setIsApp(appId);
    aWindow.gBrowser.reloadTab(tab);
    tab.style.color = priv8.colorForSandbox(aSandbox);
  });
}
...
let Priv8Overlay = {
  ...
  panelManager: function(page) {
    ...
    Services.obs.addObserver(function (aSubject, aTopic, aData) {
      ...
      self._panelManagerPage(aSubject, page);
